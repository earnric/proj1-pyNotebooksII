...
def outputHistogramData(z,i,halo):
    ...
    tempMetal = halo.s['metal']
    tempMetal[tempMetal < 1e-10] = 1e-10
    tempPmetal = halo.s['pzf']
    tempPmetal[tempPmetal < 1e-10] = 0.0
    ...
    for indx2,j in enumerate(bins):
        if indx2 < len(bins)-1 :
            ...
            primsm[indx2] = np.sum((1.0-halo.s['ppf'][cond]) * 
                                   (tempPmetal[cond] / tempMetal[cond]) * halo.s['mass'][cond])
...
#    np.savetxt("primord2SM_z%.1lf-%i.txt"%(z,i),primsm2,comments='') # comments='' gets rid of "#" in output
    np.savetxt("primordSM_z%.1lf-%i_40kpc.txt"%(z,i),primsm,comments='') # comments='' gets rid of "#" in output
    np.savetxt("pristSM_z%.1lf-%i_40kpc.txt"%(z,i),psm,comments='') # comments='' gets rid of "#" in output
    np.savetxt("totSM_z%.1lf-%i_40kpc.txt"%(z,i),tsm,comments='') # comments='' gets rid of "#" in output
    print ("Saving data...")
    
    # Update this first one with the center coords so these are all relative to the center pt.
    np.savetxt("spLocHaloOnly_%05.2lf_%i_40kpc.txt"%(z,i),halo.s['pos'],header="x\ty\tz",comments='') 
    np.savetxt("spMassHaloOnly_%05.2lf_%i_40kpc.txt"%(z,i),halo.s['mass'],header="mass",comments='') 
    np.savetxt("spZHaloOnly_%05.2lf_%i_40kpc.txt"%(z,i),halo.s['zsolar'],header="Z_solar",comments='') 
    np.savetxt("spPPFHaloOnly_%05.2lf_%i_40kpc.txt"%(z,i),halo.s['ppf'],header="ppf",comments='') 
    np.savetxt("spPZHaloOnly_%05.2lf_%i_40kpc.txt"%(z,i),halo.s['pzsolar'],header="pz_solar",comments='') 
    np.savetxt("spBTHaloOnly_%05.2lf_%i_40kpc.txt"%(z,i),halo.s['tform'].in_units('yr'),header="birth_time_yr",comments='') 
    print ("Star Particle Data Files saved for %05.1lf, %i_40kpc"%(z,i))
    return
